...
contract MarketPlace is ReentrancyGuard {
    ...
    event MarketItemSold(uint256 itemId, address indexed nftContract, address indexed seller, address indexed newOwner);

    event MarketItemUnlisted(uint256 itemId);

    event MarketItemBid(uint256 indexed itemId, address indexed bidder, uint256 amount);
    ...
    function createMarketItem(
        ...
        if (ERC165Checker.supportsInterface(nftContract, ERC2981INTERFACE)) {
            ...
            idToMarketItem[itemId] = MarketItem(itemId, nftContract, tokenId, payable(creator), payable(msg.sender), payable(address(0)), price, royaltyAmount, false, false);
        } else {
    ...
    function createMarketAuction(
        ...
        if (ERC165Checker.supportsInterface(nftContract, ERC2981INTERFACE)) {
            ...
            idToMarketItem[itemId] = MarketItem(itemId, nftContract, tokenId, payable(creator), payable(msg.sender), payable(address(0)), floorPrice, royaltyAmount, true, false);
        } else {
            address creator = msg.sender;
            uint256 royaltyAmount = 0;
            idToMarketItem[itemId] = MarketItem(itemId, nftContract, tokenId, payable(creator), payable(msg.sender), payable(address(0)), floorPrice, royaltyAmount, true, false);
        ...
        emit MarketItemCreated(itemId, nftContract, tokenId, msg.sender, address(0), floorPrice);
    }
    ...
    function createAuctionSale(address nftContract, uint256 itemId) external payable nonReentrant {
        MarketItem storage currentItem = idToMarketItem[itemId];
        AuctionInfo storage currentInfo = auctionData[itemId];
        require(!currentItem.sold, 'Item has already been sold');
        require(currentItem.isAuction, 'Item is not for auction');
    ...
    function unlistItem(uint256 itemId) external nonReentrant {
        ...
        _itemsSold.increment();
        emit MarketItemUnlisted(itemId);
    }
}
